...
bool L::OpenFile(const wchar_t* wszFileName)
{
	...
	// ѕровер€ем, €вл€етс€ ли путь абсолютным (начинаетс€ с буквы диска и двоеточи€, например, "C:")
	if (wszFileName[1] == L':' && (wszFileName[2] == L'\\' || wszFileName[2] == L'/'))
	{
		// ≈сли путь абсолютный, используем его напр€мую
		CRT::StringCopy(wszFilePath, wszFileName);
		L::stream(LOG_INFO) << "Using absolute path: " << wszFileName;
	}
	else
	{
		// ≈сли путь относительный, добавл€ем его к текущей рабочей директории
		if (!CORE::GetWorkingPath(wszFilePath))
		{
			L::stream(LOG_ERROR) << "Failed to get working path.";
			return false;
		}

		CRT::StringCat(wszFilePath, wszFileName);
		L::stream(LOG_INFO) << "Using relative path: " << wszFilePath;
	}

	// ќткрываем файл
	L::stream(LOG_INFO) << "Attempting to create file: " << wszFilePath;
	if (hFileStream = ::CreateFileW(wszFilePath, GENERIC_WRITE, FILE_SHARE_READ, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr); hFileStream == INVALID_HANDLE_VALUE)
	{
		...
		L::stream(LOG_ERROR) << "Failed to create file. Error code: " << dwError;
		return false;
	}

	// ƒобавл€ем UTF-8 BOM
	::WriteFile(hFileStream, "\xEF\xBB\xBF", 3UL, nullptr, nullptr);
	L::stream(LOG_INFO) << "File created successfully.";
