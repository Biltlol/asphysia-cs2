...
// Forward declaration для CSkeletonInstance
class CSkeletonInstance;

// Определяем CGameSceneNode
...
// Определяем CSkeletonInstance после CGameSceneNode
...
// Временно убираем CHitboxComponent и Hitbox_t
// class CHitboxComponent
// {
// public:
//     CS_CLASS_NO_INITIALIZER(CHitboxComponent);

//     SCHEMA_ADD_FIELD(Hitbox_t*, GetHitboxes, "CHitboxComponent->m_hitboxes");
//     SCHEMA_ADD_FIELD(int, GetHitboxCount, "CHitboxComponent->m_hitboxCount");
// };

// struct Hitbox_t
// {
//     int m_boneIndex;    // 0x00
//     Vector_t m_vecMins; // 0x04
//     Vector_t m_vecMaxs; // 0x10
...
public:
    ...
    // Получить позицию хитбокса (временная заглушка)
    [[nodiscard]] Vector_t GetHitboxPosition(int nHitbox)
    {
        if (!this)
            return Vector_t(0.f, 0.f, 0.f);

        // Получаем GameSceneNode
        CGameSceneNode* pGameSceneNode = this->GetGameSceneNode();
        if (!pGameSceneNode)
            return Vector_t(0.f, 0.f, 0.f);

        // Получаем SkeletonInstance
        CSkeletonInstance* pSkeleton = pGameSceneNode->GetSkeletonInstance();
        if (!pSkeleton || !pSkeleton->pBoneCache || pSkeleton->nBoneCount <= 0)
            return Vector_t(0.f, 0.f, 0.f);

        // Временная заглушка: используем индекс кости напрямую
        // @todo: заменить на реальные данные хитбоксов после дампа CHitboxComponent и Hitbox_t
        int nBone = nHitbox; // Предполагаем, что nHitbox соответствует индексу кости

        // Проверяем, что кость валидна
        if (nBone < 0 || nBone >= pSkeleton->nBoneCount)
            return Vector_t(0.f, 0.f, 0.f);

        // Получаем матрицу кости (Matrix2x4_t)
        Matrix2x4_t boneMatrix = pSkeleton->pBoneCache[nBone];

        // Получаем позицию кости
        Vector_t vecBonePos = boneMatrix.GetOrigin(0);

        // @todo: добавить смещение центра хитбокса и вращение после получения реальных данных
        return vecBonePos;
    }

    // SCHEMA_ADD_FIELD(CHitboxComponent*, GetHitboxComponent, "C_CSPlayerPawn->m_hitboxComponent");
