...
void T::RageBot()
{
	ImGui::BeginChild(CS_XOR("ragebot.aimbot"), ImVec2{}, true, ImGuiWindowFlags_MenuBar);
	{
		...
		{
			// Клавиша активации
			ImGui::HotKey(CS_XOR("activation key"), &C_GET(unsigned int, Vars.nRageBotActivationKey));

			// FOV
			ImGui::SliderFloat(CS_XOR("field of view (FOV)"), &C_GET(float, Vars.flRageBotFOV), 1.f, 180.f, CS_XOR("%.1f degrees"));

			// Выбор хитбокса
			static const char* arrHitboxes[] = { "head", "chest", "pelvis" };
			ImGui::Combo(CS_XOR("hitbox"), &C_GET(int, Vars.nRageBotHitbox), arrHitboxes, IM_ARRAYSIZE(arrHitboxes));

			// Silent aim
			ImGui::Checkbox(CS_XOR("silent aim"), &C_GET(bool, Vars.bRageBotSilent));

			// Hitchance
			ImGui::SliderFloat(CS_XOR("hitchance"), &C_GET(float, Vars.flRageBotHitchance), 0.f, 100.f, CS_XOR("%.1f%%"));
		}
		ImGui::EndDisabled();
	}
	ImGui::EndChild();
