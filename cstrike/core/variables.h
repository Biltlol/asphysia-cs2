...
using VisualHitbox_t = int;
enum EVisualHitbox : VisualHitbox_t
{
    VISUAL_HITBOX_HEAD = 0,
    VISUAL_HITBOX_CHEST,
    VISUAL_HITBOX_PELVIS,
    VISUAL_HITBOX_MAX
};

// Добавляем перечисление для хитбоксов
enum EHitbox : int
{
    HITBOX_HEAD = 0,
    HITBOX_CHEST = 1,
...
#pragma region variables_ragebot
    ...
    C_ADD_VARIABLE(unsigned int, nRageBotActivationKey, VK_LBUTTON); // По умолчанию левая кнопка мыши
    C_ADD_VARIABLE(float, flRageBotFOV, 30.f); // FOV по умолчанию 30 градусов
    C_ADD_VARIABLE(int, nRageBotHitbox, VISUAL_HITBOX_HEAD); // По умолчанию голова
    C_ADD_VARIABLE(bool, bRageBotSilent, false); // Silent aim
    C_ADD_VARIABLE(float, flRageBotHitchance, 80.f); // Hitchance по умолчанию 80%
#pragma endregion
};
